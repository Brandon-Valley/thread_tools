...
class Simple_Thread_Manager:
    ...
    def __exit__(self, type, value, traceback):
        ...
        for fut in self.futures:
            # If thread raised an error, calling that thread's .result will raise the error
            _ = fut.result()
...
if __name__ == "__main__":
    ...
    with Simple_Thread_Manager(THREADING_ENABLED, NUM_CORES) as stm:
        for str1, str2 in STR_PAIRS_TO_PRINT_LIST:
            stm.thread_func_if_enabled(print_2_strings, str1, str2)

    ################################################################################################
    # TEST #2 - Threading Disabled
    ################################################################################################
    print("\n\nTEST #2 - Threading Disabled\n")
    with Simple_Thread_Manager(False, NUM_CORES) as stm:
        for str1, str2 in STR_PAIRS_TO_PRINT_LIST:
            stm.thread_func_if_enabled(print_2_strings, str1, str2)
